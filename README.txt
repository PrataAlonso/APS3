Atividade Pratica Supervisionada do terceiro semestre da UNIP
--------------------------------------------------------------------------------------
Utilizado linguagem Java/Java Swing com a IDE NetBeans v18
Utilizado banco de dados MySQL com MySQL Workbench
--------------------------------------------------------------------------------------
Codigo por João Victor Prata Alonso e Bruno Benedittini
Banco de dados por Leonardo de Souza Grao e Leonardo Assumpçao Gonçalves
--------------------------------------------------------------------------------------
Data de finalização: 24/05/2024 as 18:38pm
--------------------------------------------------------------------------------------

-----------------------------------------IMPORTANTE---------------------------------------------
Ao extrair o arquivo .rar, entre em sua IDE e selecione o projeto. No projeto, clique em
propriedades e vá nas bibliotecas onde você adicionará no classpath o driver necessário.

a Agencia e a senha para realizar o login como USUARIO é: AB12C4 e user25908
para realizar o login como ADMIN é: admin e admin
------------------------------------------------------------------------------------------
Uso permitido para apresentação e curriculo.
